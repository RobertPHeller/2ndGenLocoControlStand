...
void Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
    ...
    while (n--) {
        ...
        buff[i++] = pgm_read_byte(c++);
    }
    if (i > 1) ::write(i2cfd_,buff,i);
}
...
*/
bool Adafruit_SSD1306::begin(const char *i2cpath, uint8_t vcs, uint8_t addr, 
                             bool reset, bool periphBegin) {

  i2cfd_ = ::open(i2cpath, O_RDWR);
  HASSERT(i2cfd_ >= 0);
  ::ioctl(i2cfd_, I2C_SLAVE, addr);
...
void Adafruit_SSD1306::display(void) {
  ...
  if (bytesOut>1) ::write(i2cfd_,buff,bytesOut);
}
